...
from utils.resume_parser import resume_links
...
async def build_tailored_resume(job: dict, *, user_name: str = None, user_email: str = None,
    ...
    {"text", "links", "cached", "ok", "issues", "rate_limited"} — text is None when the
    ...
    if not force and job_id:
        ...
        if cached and stored.get("tailored_resume_version") == version:
            return {"text": cached, "links": links, "cached": True, "ok": True,
    ...
    if limited:
        return {"text": None, "links": links, "cached": False, "ok": False,
    ...
    if not resume:
        return {"text": text, "links": links, "cached": False, "ok": True,
                "issues": [], "rate_limited": False}
    ...
    return {"text": v["text"], "links": links, "cached": False, "ok": True,
            "issues": v["issues"], "rate_limited": False}
